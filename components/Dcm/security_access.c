...
u8 SEED_1[SEED_SIZE] = {1, 2, 3, 4, 5, 6, 7, 8};
u8 SEED_3[SEED_SIZE] = {11, 12, 13, 14, 15, 16, 17, 18};
u8 SEED_5[SEED_SIZE] = {21, 22, 23, 24, 25, 26, 27, 28};
u8 SEED_7[SEED_SIZE] = {31, 32, 33, 34, 35, 36, 37, 38};

u8 KEY_1[SEED_SIZE] = {1, 2, 3, 4, 5, 6, 7, 8};
u8 KEY_3[SEED_SIZE] = {11, 12, 13, 14, 15, 16, 17, 18};
u8 KEY_5[SEED_SIZE] = {21, 22, 23, 24, 25, 26, 27, 28};
u8 KEY_7[SEED_SIZE] = {31, 32, 33, 34, 35, 36, 37, 38};

u8 SEED[SEED_SIZE] = {1, 2, 3, 4, 5, 6, 7, 8};
u8 KEY[SEED_SIZE] = {1, 2, 3, 4, 5, 6, 7, 8};
...
void seed_generation(u8 *resp_data, u16 *idx, u8 request_level)
{
    /* STUB */
    u8 n = (request_level - 1) / 2;
    for (u8 i = 0; i < 8; i++)
    {
...
bool seed_validation(u8 *client_key, u8 request_level)
{
    /* STUB */
    u8 n = (request_level - 1) / 2; /* Reduce odd numbers to a linear map: 0=1, 1=3, 2=5, 3=7 */
    for (u8 i = 0; i < 8; i++)
    {
...
bool handle_failure(bool result)
{
    ...
    u8 current_security_level = get_state(STATE_SECURITY_SERVICE);
...
bool handle_access_escalation(UDS_Packet *rx, u8 *resp_data, u16 *idx)
{
    u8 current_security_level = get_state(STATE_SECURITY_SERVICE);
    u8 request_security_level = rx->data[0];
    ...
    else
    {
        ...
        u8 *client_key = &rx->data[1]; /* point to the data starting after the SFB. */
...
bool security_check_security_access(UDS_Packet *rx, u8 *resp_data, u16 *idx, u8 dgSess, u8 secAcess)
{
