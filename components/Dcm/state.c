...
u8 DEVICE_STATE[3];
u64 LAST_CLIENT_CALL;
u64 LAST_SECURITY_CALL;
...
void update_state()
{
  if (DEVICE_STATE[STATE_DIAGNOSTIC_SESSION] == 0x3 || DEVICE_STATE[STATE_DIAGNOSTIC_SESSION] == 0x2)
  {
    ...
    if (!check_if_timeout(LAST_CLIENT_CALL, EXTENDED_SESSION_TIMEOUT))
    {
      DEVICE_STATE[STATE_DIAGNOSTIC_SESSION] = 0x1;
      printf("PRIVILEGE TIMEOUT\n");
    }
  }
  if (DEVICE_STATE[STATE_SECURITY_SERVICE] == 0xFF)
  {
    /* Allow security requests again. */
    if (!check_if_timeout(LAST_SECURITY_CALL, SECURITY_REQUEST_TIMEOUT))
    {
      DEVICE_STATE[STATE_SECURITY_SERVICE] = 0x00;
      printf("AVAILABLE FOR SEC TRY AGAIN\n");
    }
  }
}
